...
public class BallAndBeam extends VirtualProcess {
	...
	private RoundRectangle2D box = new RoundRectangle2D.Double(100.0, 80.0,
	...
	public void draw(Graphics2D g2, JPanel jp, double[] state, double[] input,
			double[] output) {
		if (!init) {
		...
		g2.scale(scale, scale); // scale the animation
		g2.setColor(Color.gray);
		g2.fill(box); // animate the background
		g2.setColor(Color.black);
		g2.drawString("Ball&Beam", 122, 95);
		g2.draw(box);
		g2.rotate(-1 * output[1] * Math.PI / 40, 150, 115); // rotate the
															// following with
															// the beam angle
		...
		g2.translate(output[0] * 10.0, 0.0); // move the following with the ball
												// position
